    ...
    def draw_scale(self):
        ...
        if self.draw_dial is True:
            ...
            for angle in self._frange(math.radians(self.min_angle + self.offset_angle),
                ...
                start_point = (self.gauge_origin[0] - self.radius * math.sin(angle) * 0.93,
                               self.gauge_origin[1] + self.radius * math.cos(angle) * 0.93)
                ...
                if self.dial_format is not None and self.dial_format != 'None':
                    ...
                    # Use getbbox if available, otherwise fall back to getsize
                    try:
                        left, top, right, bottom = self.dial_font.getbbox(text)
                        string_size = (right - left, bottom - top)
            ...
            if self.dial_labels is not None:
                for k in self.dial_labels.keys():
                    ...
                    if (angle >= 0.0) and (angle <= 1):
                        ...
                        # Use getbbox if available, otherwise fall back to getsize
                        try:
                            left, top, right, bottom = self.dial_label_font.getbbox(self.dial_labels[k])
                            string_size = (right - left, bottom - top)
                        except AttributeError:
    ...
    def draw_labels(self):
        ...
        if self.text_labels is not None:
            ...
            for l in self.text_labels:
                ...
                text = str(l) if not isinstance(l, str) else l
                
                # Use getbbox if available, otherwise fall back to getsize
                try:
                    left, top, right, bottom = self.text_font.getbbox(text)
